...
def vertices(filename):
    ...
    for x in f.readlines():
        x = re.sub(' +', ' ', x).split(" ")
        if x[0] == 'v':
            a.append([float(x[1]),float(x[2]),float(x[3])])
    f.close()
    return a
def meshFaces(edges,filename):
    a = vertices(filename)
    f = open(filename, 'r')
    for x in f.readlines():
        x = re.sub(' +', ' ', x).split(" ")
        if x[0] == 'f':
            ...
            while c<len(vertice):
                p0 = int(vertice[0]) - 1
                p1 = int(vertice[c - 1]) - 1
                p2 = int(vertice[c]) - 1
                add_polygon(edges, a[p0][0], a[p0][1], a[p0][2],
...
def add_pyramid( polygons, x, y, z, width, height, depth ):
    ...
    #front
    add_polygon(polygons, x, y, z, x1, y, z, a, y1, a2)

    #back
    add_polygon(polygons, x, y, z1, a, y1, a2, x1, y, z1)

    #right side
    add_polygon(polygons, x1, y, z, x1, y, z1, a, y, a2)
    #left side
    add_polygon(polygons, x, y, z1, x, y, z, a, y1, a2)
